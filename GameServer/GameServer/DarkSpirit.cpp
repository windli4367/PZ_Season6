...
void CDarkSpirit::ModeAttackRandom() // OK
{
	...
	if(IndexCount > 0)
	{
		if((rand()%100) < gServerInfo.m_DarkSpiritRangeAttackRate)
		{
			this->RangeAttack(lpObj->Index,IndexTable[rand()%IndexCount]);
		}
		else
		{
			this->SendAttackMsg(lpObj->Index,IndexTable[rand()%IndexCount],1,0);
		}
	}
}
...
bool CDarkSpirit::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,BYTE action) // OK
{
	...
	if((rand()%100) < gMasterSkillTree.GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_DARK_SPIRIT_DOUBLE_DAMAGE_RATE))
	{
		effect |= 0x40;
		damage += damage;
	}
...
bool CDarkSpirit::MissCheck(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int send,BYTE* miss) // OK
{
	...
	else
	{
		...
		if((rand()%((this->m_AttackSuccessRate==0)?1:this->m_AttackSuccessRate)) < lpTarget->DefenseSuccessRate)
		{
			gAttack.MissSend(lpObj,lpTarget,lpSkill,send,0);
			return 0;
		}
...
int CDarkSpirit::GetTargetDefense(LPOBJ lpObj,LPOBJ lpTarget,WORD* effect) // OK
{
	...
	if((rand()%100) < gMasterSkillTree.GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_DARK_SPIRIT_IGNORE_DEFENSE_RATE))
	{
		(*effect) = 1;
...
int CDarkSpirit::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	#if(GAMESERVER_UPDATE>=602)
	if((rand()%100) < (gServerInfo.m_DarkSpiritCriticalDamageRate+gMasterSkillTree.GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_DARK_SPIRIT_CRITICAL_DAMAGE_RATE)))
	#else
	if((rand()%100) < (gServerInfo.m_DarkSpiritCriticalDamageRate))
	#endif
	{
		(*effect) = 3;
	...
	#if(GAMESERVER_UPDATE>=602)
	if((rand()%100) < (gServerInfo.m_DarkSpiritExcellentDamageRate+gMasterSkillTree.GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_DARK_SPIRIT_EXCELLENT_DAMAGE_RATE)))
	#else
	if((rand()%100) < (gServerInfo.m_DarkSpiritExcellentDamageRate))
	#endif
	{
		(*effect) = 2;
