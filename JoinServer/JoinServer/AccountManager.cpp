...
void CAccountManager::DisconnectProc() // OK
{
	...
	for(std::map<std::string,ACCOUNT_INFO>::iterator it=this->m_AccountInfo.begin();it != this->m_AccountInfo.end();)
	{
		...
		gLog.Output(LOG_ACCOUNT,"[AccountInfo] Conta desconectada (Conta: %s, IP: %s, Código do GameServer: %d)",it->second.Account,it->second.IpAddress,it->second.GameServerCode);
