...
void CServerList::Load(char* path)
{
	...
	LogAdd(LOG_BLUE,"[ServerList] Lista de servidores carregada.");
...
void CServerList::MainProc()
{
	if(this->m_JoinServerState != 0 && (GetTickCount()-this->m_JoinServerStateTime) > 10000)
	{
		this->m_JoinServerState = 0;
		this->m_JoinServerStateTime = 0;
		LogAdd(LOG_RED,"[ServerList] JoinServer desligado");
	}
	...
	for(std::map<int,SERVER_LIST_INFO>::iterator it=this->m_ServerListInfo.begin();it != this->m_ServerListInfo.end();it++)
	{
		if(it->second.ServerState != 0 && (GetTickCount()-it->second.ServerStateTime) > 10000)
		{
			...
			LogAdd(LOG_BLACK,"[ServerList] GameServer desligado (%s) (%d)",it->second.ServerName,it->second.ServerCode);
		}
	}
}
...
long CServerList::GenerateServerList(BYTE* lpMsg,int* size)
{
	...
	if(this->CheckJoinServerState() != 0)
	{
		for(std::map<int,SERVER_LIST_INFO>::iterator it=this->m_ServerListInfo.begin();it != this->m_ServerListInfo.end();it++)
		{
			...
			{
				pServerList.ServerCode	= it->second.ServerCode;
				pServerList.UserTotal	= it->second.UserTotal;
				pServerList.type		= 0xCC;

				memcpy(&lpMsg[(*size)],&pServerList,sizeof(pServerList));
				(*size) += sizeof(pServerList);
...
void CServerList::GCGameServerLiveRecv(SDHP_GAME_SERVER_LIVE_RECV* lpMsg)
{
	...
	{
		LogAdd(LOG_BLACK,"[ServerList] GameServer ativo (%s) (%d)",lpServerListInfo->ServerName,lpServerListInfo->ServerCode);
	}
...
void CServerList::JCJoinServerLiveRecv(SDHP_JOIN_SERVER_LIVE_RECV* lpMsg)
{
	if(this->m_JoinServerState == 0)
	{
		LogAdd(LOG_GREEN,"[ServerList] JoinServer ativo.");
	}
