...
bool CConnection::Connect(char* IpAddress,WORD port,DWORD WinMsg) // OK
{
	...
	if(connect(this->m_socket,(SOCKADDR*)&target,sizeof(target)) == SOCKET_ERROR)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[Connection] connect() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect();
			return 0;
		}
	}
	...
	{
		gLog.Output(LOG_CONNECT,"[Connection] WSAAsyncSelect() falhou com o erro: %d",WSAGetLastError());
		this->Disconnect();
		return 0;
	}
...
bool CConnection::DataRecv() // OK
{
	...
	if((result=recv(this->m_socket,(char*)&this->m_RecvBuff[this->m_RecvSize],(MAX_BUFF_SIZE-this->m_RecvSize),0)) == SOCKET_ERROR)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[Connection] recv() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect();
			return 0;
		}
	}
...
bool CConnection::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	while(size > 0)
	{
		if((result=send(this->m_socket,(char*)&lpMsg[count],size,0)) == SOCKET_ERROR)
		{
			...
			{
				gLog.Output(LOG_CONNECT,"[Connection] send() falhou com o erro: %d",WSAGetLastError());
				this->Disconnect();
				this->m_critical.unlock();
				return 0;
			}
