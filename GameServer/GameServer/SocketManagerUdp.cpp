...
bool CSocketManagerUdp::Start(WORD port) // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManagerUdp] WSASocket() falhou com o erro: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManagerUdp] bind() falhou com o erro: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManagerUdp] CreateThread() falhou com o erro: %d",GetLastError());
		this->Clean();
		return 0;
	}
...
bool CSocketManagerUdp::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(this->m_SocketAddr.sin_addr.s_addr == INADDR_NONE)
	{
		...
		if(host == 0)
		{
			gLog.Output(LOG_CONNECT,"[SocketManagerUdp] gethostbyname() falhou com o erro: %d",WSAGetLastError());
			this->Clean();
			return 0;
		}
...
bool CSocketManagerUdp::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	if(result == SOCKET_ERROR)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManagerUdp] sendto() falhou com o erro: %d",WSAGetLastError());
			memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
			this->m_SendSize = 0;
			return 0;
		}
...
DWORD WINAPI CSocketManagerUdp::ServerRecvThread(CSocketManagerUdp* lpSocketManagerUdp) // OK
{
	while(true)
	{
		...
		if(result == SOCKET_ERROR)
		{
			gLog.Output(LOG_CONNECT,"[SocketManagerUdp] recvfrom() falhou com o erro: %d",GetLastError());
