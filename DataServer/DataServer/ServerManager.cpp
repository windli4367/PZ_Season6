...
void CServerManager::AddServer(int index,char* ip,SOCKET socket) // OK
{
	...
	LogAdd(LOG_BLACK,"[ServerManager][%d] Servidor adicionado (%s)",this->m_index,this->m_IpAddr);
}

void CServerManager::DelServer() // OK
{
	LogAdd(LOG_BLACK,"[ServerManager][%d] Servidor renmovido (%s)",this->m_index,this->m_IpAddr);
...
void CServerManager::SetServerInfo(char* name,WORD port,WORD code) // OK
{
	...
	LogAdd(LOG_BLACK,"[ServerManager][%d] Informação do Servidor (%s) (%d) (%d)",this->m_index,this->m_ServerName,this->m_ServerPort,this->m_ServerCode);
}
