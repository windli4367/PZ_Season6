...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	char buff[256];
	wsprintf(buff,"[%s][%s][%d] %s (Iniciando)",GAMESERVER_VERSION,gServerInfo.m_ServerVersion,gServerInfo.m_ServerCode,gServerInfo.m_ServerName);
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		{
			LogAdd(LOG_RED,"Impossível iniciar o GameServer");
		}
		else
		{
			GameMainInit(hWnd);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Deseja encerrar o GameServer?","Encerrar GameServer",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						DestroyWindow(hWnd);
					}
					break;
