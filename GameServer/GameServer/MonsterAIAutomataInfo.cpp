	...
	for(int n=0;n < this->m_AIStateTransCount[lpObj->CurrentAIState];n++)
	{
		...
		switch(lpMonsterAIState->m_TransitionType)
		{
			case MAI_STATE_TRANS_NO_ENEMY:
				...
				{
					if((rand()%100) < lpMonsterAIState->m_TransitionRate)
					{
			...
			case MAI_STATE_TRANS_IN_ENEMY:
				if(OBJECT_RANGE(lpObj->TargetNumber) != 0)
				{
					if(gObjCalcDistance(lpObj,&gObj[lpObj->TargetNumber]) <= lpObj->AttackRange && (rand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
			...
			case MAI_STATE_TRANS_AGRO_UP:
				if(OBJECT_RANGE(lpObj->TargetNumber) != 0)
				{
					if(lpObj->Agro.GetAgro(lpObj->TargetNumber) >= lpMonsterAIState->m_TransitionValue && (rand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
			...
			case MAI_STATE_TRANS_GROUP_SOMMON:
				...
				{
					if(CMonsterAIGroup::FindGroupMemberToSommon(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid) != 0 && (rand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
			...
			case MAI_STATE_TRANS_GROUP_HEAL:
				if(lpObj->GroupNumber >= 0 && lpObj->GroupNumber < MAX_MONSTER_AI_GROUP)
				{
					if(CMonsterAIGroup::FindGroupMemberToHeal(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid,6,lpMonsterAIState->m_TransitionValue) != 0 && (rand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
