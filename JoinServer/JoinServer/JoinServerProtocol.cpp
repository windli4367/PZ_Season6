...
void GJConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg,int index) // OK
{
	...
	if(gAccountManager.GetAccountCount() >= MAX_ACCOUNT) // TODO: Test
	{
		pMsg.result = 4;
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	...
	BOOL AcccountCreated = FALSE;

	if(MD5Encryption == FALSE)
	{
		if((gQueryManager.ExecQuery("SELECT memb__pwd FROM MEMB_INFO WHERE memb___id = '%s'",lpMsg->account) == false) || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			if((EnableAutoReg == TRUE) && (strlen(lpMsg->account) >= AutoRegMinLen) && (strlen(lpMsg->password) >= AutoRegMinLen))
			{
				gQueryManager.Close();

				AcccountCreated = gQueryManager.ExecQuery
				(
					"INSERT INTO dbo.MEMB_INFO (memb___id, memb__pwd, memb_name, sno__numb, ctl1_code, bloc_code, AccountLevel) VALUES ('%s', '%s', '%s', '0', 0, 0, 0)",
					lpMsg->account,
					lpMsg->password,
					lpMsg->account
				);
	...
	gLog.Output(LOG_ACCOUNT,"[AccountInfo] Conta conectada (Login: %s, IP: %s, GameServerCode: %d)",AccountInfo.Account,AccountInfo.IpAddress,AccountInfo.GameServerCode);
}
...
void GJDisconnectAccountRecv(SDHP_DISCONNECT_ACCOUNT_RECV* lpMsg,int index) // OK
{
	...
	gLog.Output(LOG_ACCOUNT,"[AccountInfo] Conta desconectada (Login: %s, IP: %s, GameServerCode: %d)",AccountInfo.Account,AccountInfo.IpAddress,AccountInfo.GameServerCode);
}
