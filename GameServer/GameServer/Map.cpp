...
void CMap::GetMapPos(int map,short* ox,short* oy) // OK
{
	...
	for(int n=0;n < 50;n++)
	{
		int x = this->m_MapRespawn[map].left+(rand()%(this->m_MapRespawn[map].right-this->m_MapRespawn[map].left));
		int y = this->m_MapRespawn[map].top+(rand()%(this->m_MapRespawn[map].bottom-this->m_MapRespawn[map].top));
