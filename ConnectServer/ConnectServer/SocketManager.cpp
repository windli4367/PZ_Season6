...
bool CSocketManager::Start(WORD port)
{
	...
	LogAdd(LOG_BLACK,"[SocketManager] Servidor iniciado na porta [%d]",this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket()
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] WSASocket() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketManager] bind() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}

	if(listen(this->m_listen,5) == SOCKET_ERROR)
	{
		LogAdd(LOG_RED,"[SocketManager] listen() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort()
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] socket() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}

	if((this->m_CompletionPort=CreateIoCompletionPort((HANDLE)socket,0,0,0)) == 0)
	{
		LogAdd(LOG_RED,"[SocketManager] CreateIoCompletionPort() falhou com o erro: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread()
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] CreateThread() falhou com o erro: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(this->m_ServerAcceptThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		LogAdd(LOG_RED,"[SocketManager] SetThreadPriority() falhou com o erro: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue()
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] CreateSemaphore() falhou com o erro: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size)
{
	...
	if(WSASend(lpClientManager->m_socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManager] WSASend() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
void CSocketManager::Disconnect(int index)
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] closesocket() falhou com o erro: %d",WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext)
{
	...
	if(WSARecv(lpClientManager->m_socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			LogAdd(LOG_RED,"[SocketManager] WSARecv() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager)
{
	...
	while(true)
	{
		...
		if(socket == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			LogAdd(LOG_RED,"[SocketManager] WSAAccept() falhou com o erro: %d",WSAGetLastError());
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager)
{
	...
	while(true)
	{
		if(GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort,&IoSize,&index,&lpOverlapped,INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				LogAdd(LOG_RED,"[SocketManager] GetQueuedCompletionStatus() falhou com o erro: %d",GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager)
{
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManager] WaitForSingleObject() falhou com o erro: %d",GetLastError());
			break;
		}
