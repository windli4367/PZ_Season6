...
bool CSocketManagerUdp::Start(WORD port) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] WSASocket() falhou com o erro: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] bind() falhou com o erro: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] CreateThread() falhou com o erro: %d",GetLastError());
		this->Clean();
		return 0;
	}
...
bool CSocketManagerUdp::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(this->m_SocketAddr.sin_addr.s_addr == INADDR_NONE)
	{
		...
		if(host == 0)
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] gethostbyname() falhou com o erro: %d",WSAGetLastError());
			this->Clean();
			return 0;
		}
...
bool CSocketManagerUdp::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	if(result == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] sendto() falhou com o erro: %d",WSAGetLastError());
			memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
			this->m_SendSize = 0;
			return 0;
		}
...
DWORD WINAPI CSocketManagerUdp::ServerRecvThread(CSocketManagerUdp* lpSocketManagerUdp) // OK
{
	while(true)
	{
		...
		if(result == SOCKET_ERROR)
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] recvfrom() falhou com o erro: %d",GetLastError());
