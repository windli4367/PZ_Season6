...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	char buff[256];
	wsprintf(buff,"[%s][%d] DataServer (Iniciando)",DATASERVER_VERSION,DATASERVER_UPDATE);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Deseja encerrar o DataServer?","Encerrar DataServer",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						DestroyWindow(hWnd);
					}
					break;
