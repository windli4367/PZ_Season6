...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] WSASocket() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] bind() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}

	if(listen(this->m_listen,5) == SOCKET_ERROR)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] listen() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] socket() falhou com o erro: %d",WSAGetLastError());
		return 0;
	}

	if((this->m_CompletionPort=CreateIoCompletionPort((HANDLE)socket,0,0,0)) == 0)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateIoCompletionPort() falhou com o erro: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateThread() falhou com o erro: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(this->m_ServerAcceptThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] SetThreadPriority() falhou com o erro: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateSemaphore() falhou com o erro: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	if(WSASend(gObj[index].Socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WSASend() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] closesocket() falhou com o erro: %d",WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(WSARecv(lpObj->Socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WSARecv() falhou com o erro: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		if(socket == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			gLog.Output(LOG_CONNECT,"[SocketManager] WSAAccept() falhou com o erro: %d",WSAGetLastError());
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] gObjAdd() falhou com o erro: %d",GetLastError());
			closesocket(socket);
			lpSocketManager->m_critical.unlock();
			continue;
		}
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		if(GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort,&IoSize,&index,&lpOverlapped,INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				gLog.Output(LOG_CONNECT,"[SocketManager] GetQueuedCompletionStatus() falhou com o erro: %d",GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager) // OK
{
	while(true)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WaitForSingleObject() falhou com o erro: %d",GetLastError());
			break;
		}
