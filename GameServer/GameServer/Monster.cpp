...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Level >= 25 && (rand()%1000000) < SetItemDropRate)
	{
		gSetItemType.MakeRandomSetItem(lpTarget->Index,lpObj->Map,lpObj->X,lpObj->Y);
		return;
	}

	if(lpObj->Level >= 25 && (rand()%1000000) < ExcItemDropRate)
	{
		DropLevel -= 25;
		ExcItemDrop = 1;
	}

	if((DropIndex=gMonsterManager.GetMonsterItem(DropLevel,ExcItemDrop,gMapManager.GetMapSocketItemDrop(lpObj->Map))) >= GET_ITEM(0,0) && (rand()%((lpObj->ItemRate==0)?1:lpObj->ItemRate)) < ItemDropRate)
	{
	...
	else if(lpObj->Money > 0 && (rand()%((lpObj->MoneyRate==0)?1:lpObj->MoneyRate)) < 10)
	{
...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	lpObj->ElementalAttribute = ((lpInfo->ElementalAttribute==6)?((rand()%5)+1):lpInfo->ElementalAttribute);
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else if(lpObj->Class == 149 || lpObj->Class == 179 || lpObj->Class == 187 || lpObj->Class == 195 || lpObj->Class == 265 || lpObj->Class == 273)
	{
		...
		pMsg.skillL = SET_NUMBERLB((0+(rand()%2)));
		...
		pMsg.skill[1] = SET_NUMBERLB((0+(rand()%2)));
	...
	else if(lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275 || lpObj->Class == 338)
	{
		...
		pMsg.skillL = SET_NUMBERLB((1+(rand()%6)));
		...
		pMsg.skill[1] = SET_NUMBERLB((1+(rand()%6)));
	...
	else if(lpObj->Class == 561)
	{
		...
		pMsg.skillL = SET_NUMBERLB((((rand()%100)>=25)?1:0));

		#else

		pMsg.skill[0] = SET_NUMBERHB(0);

		pMsg.skill[1] = SET_NUMBERLB((((rand()%100)>=25)?1:0));
...
void gObjMonsterMoveAction(LPOBJ lpObj)
{
	...
	while ( searchc-- != 0 )
	{
		__try
		{
			tpx = (lpObj->X - lpObj->MoveRange) + (BYTE)(rand()%maxmoverange);
			tpy = (lpObj->Y - lpObj->MoveRange) + (BYTE)(rand()%maxmoverange);
		}
